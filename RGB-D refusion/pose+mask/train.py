...
from loss_functions import smooth_loss, explainability_loss, residual_mask, consensus_loss
...
def train(train_loader, mask_net, pose_net, optimizer, epoch_size, train_writer):
    ...
    for i, (rgb_tgt_img, rgb_ref_imgs, depth_tgt_img, depth_ref_imgs, intrinsics, intrinsics_inv, pose_list) in enumerate(tqdm(train_loader)):
        ...
        Res_mask, ref_img_warped, diff = residual_mask(explainability_mask[0], rgb_tgt_img_var, rgb_ref_imgs_var, intrinsics_var, intrinsics_inv_var, depth_tgt_img_var, pose)
        
        # print(Res_mask[0].size(), explainability_mask[0].size())
        loss3  = consensus_loss(explainability_mask[0], Res_mask)
        ...
        if n_iter % (args.training_output_freq) == 0:
            ...
            train_writer.add_image('train Res mask ', 
                                    tensor2array(Res_mask[0][0].data.cpu(), max_value=1, colormap='bone'), n_iter)
