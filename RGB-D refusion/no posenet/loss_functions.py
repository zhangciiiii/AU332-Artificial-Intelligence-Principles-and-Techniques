...
def residual_mask(tgt_img, ref_imgs, intrinsics, intrinsics_inv, depth, pose, rotation_mode='euler', padding_mode='zeros', qch=0.5, wssim=0.5):
    def one_scale(depth):
        ...
        for i, ref_img in enumerate(ref_imgs_scaled): # i=0, i=1
            ...
            mean_reconstruction_loss  = ((1- wssim)*robust_l1_per_pix(diff, q=qch) + wssim*ssim_loss) .sum(1).sum(1).sum(1)/(3*h*w) # [B]
