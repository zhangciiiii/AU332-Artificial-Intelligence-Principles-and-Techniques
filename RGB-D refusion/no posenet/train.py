...
def main():
    ...
    parameters = chain(mask_net.parameters())
    ...
    for epoch in tqdm(range(args.epochs)):
        train_loss = train(train_loader, mask_net, optimizer, args.epoch_size, training_writer)
...
def train(train_loader, mask_net, optimizer, epoch_size, train_writer):
    global args, n_iter
    ...
    for i, (rgb_tgt_img, rgb_ref_imgs, depth_tgt_img, depth_ref_imgs, intrinsics, intrinsics_inv, pose_list) in enumerate(tqdm(train_loader)):
        ...
        Res_mask, ref_img_warped, diff = residual_mask(rgb_tgt_img_var, rgb_ref_imgs_var, intrinsics_var, intrinsics_inv_var, depth_tgt_img_var, pose_list_var)
