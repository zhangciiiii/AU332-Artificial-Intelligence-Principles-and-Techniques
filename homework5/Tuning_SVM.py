...
# SVM
def testSVM(features, labels, features_test, kernel, C):
    model = SVC(kernel=kernel,random_state=0 ,C=C,gamma=0.8)
    ...
    with open("SVM_tuning_process.csv", "a") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([kernel, C, c])
        print([kernel, C, c])
...
if __name__ == '__main__':
    ...
    with open("SVM_tuning_process.csv", "w", newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['kernel', 'C','accuracy'])
    ...
    for kernel in ['linear', 'poly', 'rbf', 'sigmoid', 'precomputed' ]:
        for C in arange(0.1,1,0.1):
            testSVM(features, labels, features_test, kernel, C)
            '''
