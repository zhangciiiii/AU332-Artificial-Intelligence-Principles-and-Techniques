...
def load_csv_data(filename):
    ...
    data = file[['Elevation', 'Aspect', 'Slope', 'Horizontal_Distance_To_Hydrology', 'Vertical_Distance_To_Hydrology',
                 ...
                 'Soil_Type2', 'Soil_Type3', 'Soil_Type4', 'Soil_Type10', 'Soil_Type11',
...
# LogisticRegression
def testRandomForest(features, labels, features_test, n_estimators, criterion = 'gini'):

    #parameters = {'n_neighbors':range(1,5),'weights':['uniform','distance']}
    rf2 = ensemble.RandomForestClassifier(n_estimators=n_estimators, class_weight = 'balanced', n_jobs=-1, random_state=44, criterion = criterion)
    #rf2 = LogisticRegression(n_jobs=-1,random_state=53,C=1)
    ...
    k = 5
    c = cross_val_score(rf2, features, labels, cv = k)
    c = np.mean(c)
    print('Traing accuracy of', k, '-fold validation is: %.03f' % c)
    with open("RF_tuning_process.csv", "a") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([n_estimators, criterion,c])
        print([n_estimators, criterion,c])
...
if __name__ == '__main__':
    print('RF:\r')
    with open("RF_tuning_process.csv", "w", newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['n_estimators', 'criterion','accuracy'])
    ...
    for criterion in ['entropy', 'gini']:
        for n_estimators in range(100,1000,50):
            ...
            testRandomForest(features, labels, features_test, n_estimators, criterion)
            '''
